...
int runTheWorld ()
{
    ...
    showGuardians(guardianList,NB_GUARDIAN,myRenderer); // the static guardians
    ...
    while ( !run ) // infinite loop cause we want moving bees
    {
        ...
        showQueen(queenList,myRenderer); // the moving queen
        iHoney = showDroneAndForager(NB_FORAGER,NB_DRONE,myRenderer); // the moving foragers
    }
